    ...
    return (
        <div style={{backgroundColor: "#E3E3E3"}}>
            ...
            <Card sx={{ width: 1 }}>
                <CardHeader 
                    avatar={(
                        user.profilImage !== undefined && <Avatar src={"http://localhost:3050/Images/" + user.profilImage} alt="user" key={user.id} sx={{ width: 100, height: 100 }}/>)}
            ...
            <List>
                ...
                posts.length > 0 && posts.slice().reverse().map((post) => {
                    if(user && user.id === post.userId) return <PostCard post={post} user={user} key={post.id} />
                    else return null
                })}
            </List>
        </div>
